# -*- coding: utf-8 -*-
"""
Created on Fri Aug  2 10:10:00 2024
...
def parse_log_file(file_path):
    """Parse the log file and extract successful transactions."""
    ...
    try:
        ...
        for transaction in transactions:
            ...
            # Check if transaction contains "Document posted successfully:"
            if any("Document posted successfully:" in line for line in lines):
                success_logs = []
                for line in lines[1:]:
                    if line.strip():
                        success_logs.append(line.strip())
                successful_transactions.append((transaction_number, success_logs))
...
def write_to_xlsx(all_transactions, output_file):
    """Write the successful transactions to a single XLSX file."""
    try:
        ...
        # Write the header
        header = ['File Name', 'Transaction Number', 'Success Log 1', 'Success Log 2', 'Success Log 3']
        ws.append(header)

        for file_name, transaction_number, success_logs in all_transactions:
            # Ensure the row has enough columns (3 for success logs)
            row = [file_name, transaction_number] + success_logs[:3] + [''] * (3 - len(success_logs[:3]))
            ws.append(row)
    ...
    try:
        for filename in os.listdir(input_dir):
            if filename.endswith('.pdf'):
                ...
                successful_transactions = parse_log_file(input_file_path)
                all_transactions.extend([(filename, trans_num, success_logs) for trans_num, success_logs in successful_transactions])
